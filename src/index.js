...
function collectDOMStat(root) {
    ...
    // функция проверки классов
    function checkClasses(node) {
        for (var i = 0; i < node.classList.length; ++i) {
            if(stat.classes.hasOwnProperty(node.classList[i])) {
                stat.classes[node.classList[i]]++;
            } else {
                stat.classes[node.classList[i]] = 1;
            }
        }
    }
    ...
    // проверка текстовых элементов и углубление по дереву
    function writeStat(node) {
        for (var i = 0; i < node.childNodes.length; ++i) {
            if (node.childNodes[i].nodeType == 3) {
                stat.texts++;
            } else  {
                checkTag(node.childNodes[i]);
                checkClasses(node.childNodes[i]);
                writeStat(node.childNodes[i]);
            }
        }
    }
    ...
    var observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                ...
                for (var i = 0; i < mutation.addedNodes.length; ++i) {
                    ...
                    if (i + 1 >= mutation.addedNodes.length) {
                        fn({ type: 'insert', nodes: nodes });
                        nodes = [];
                    }
                }
                ...
                for (var i = 0; i < mutation.removedNodes.length; ++i) {
                    ...
                    if (i + 1 >= mutation.removedNodes.length) {
                        fn({ type: 'remove', nodes: nodes });
                        nodes = [];
                    }
                }
            });    
