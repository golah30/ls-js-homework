...
describe('ДЗ 7.2 - Cookie editor', () => {
    ...
    describe('Интеграционное тестирование', () => {
        ...
        filterNameInput,
        Element,
        'id элемента должен быть filter-name-input'
      );
            assert.isNotNull(addNameInput, 'элемент не найден');
            assert.instanceOf(
        addNameInput,
        Element,
        'id элемента должен быть add-name-input'
      );
            assert.isNotNull(addValueInput, 'элемент не найден');
            assert.instanceOf(
        addValueInput,
        Element,
        'id элемента должен быть add-value-input'
      );
            assert.isNotNull(addButton, 'элемент не найден');
            assert.instanceOf(
        addButton,
        Element,
        'id элемента должен быть add-button'
      );
            assert.isNotNull(listTable, 'элемент не найден');
            assert.instanceOf(
        listTable,
        Element,
        'id элемента должен быть list-table'
      );
        });
        ...
        it('cookie должны добавляться при нажатии на "добавить"', () => {
            ...
            cookies = getCookies();
            assert.isTrue(
        cookies.hasOwnProperty(addNameInput.value),
        'cookie не добавлена в барузер'
      );
            assert.equal(
        cookies[addNameInput.value],
        addValueInput.value,
        'cookie не добавлена в барузер'
      );
        ...
        it('если при добавлении указано имя существующей cookie, то в таблице должно быть изменено ее значение', () => {
            ...
            changedRow = rows.filter(
        row => row.children[1].textContent.trim() == 'other-test-cookie-value-2'
      );
            assert.lengthOf(
        changedRow,
        1,
        'новое значение для cookie не найдено в таблице'
      );
        });
        ...
        describe('Фильтрация', () => {
            ...
            it('добавлять cookie в таблицу, только если имя cookie соответствует фильтру', () => {
                ...
                assert(
          cookies.hasOwnProperty(addNameInput.value),
          'должна быть добавлена в браузер'
        );
                assert.equal(
          cookies[addNameInput.value],
          addValueInput.value,
          ...
          listTable.children,
          2,
          'должна быть в таблице т.к. соответствует фильтру'
        );
            });
          ...
          listTable.children,
          1,
          'не должна быть в таблице т.к. не соответствует фильтру'
        );
            });
            ...
            it('удалить cookie из табилицы, если ее значение перестало соответствовать фильтр', () => {
                ...
                assert(
          cookies.hasOwnProperty(addNameInput.value),
          'должна оставться в браузере'
        );
                assert.equal(
          cookies[addNameInput.value],
          addValueInput.value,
          'значение в браузере должно измениться'
        );
                assert.lengthOf(
          listTable.children,
          1,
          'уже не соответствует фильтру и не должна быть в таблице'
        );
            });
